...
//-----|-----------
| 1    | 2d4 gold pieces
| 2    | <<Spellbooks,spellbook>> with 2 <<usage_points,usage point>>
...
Light Melee Weapons::
A light weapon costs 5 gold pieces and takes up 1 <<item_slots,item slot>>.

Heavy Melee Weapons::
A two-handed weapon costs 6 gold pieces and takes up 2 <<item_slots>>.
...
[IMPORTANT]
====
* Moves do not carry over to the next round; use them or lose them.
...
* Effects and conditions, such as being <<prone>> or wearing broken armor
  ...
  zero. So you can always convert your main action into moves and get three
  moves that way.
====
...
Procedure::
* You spend 10 minutes chanting and reading from the spellbook. Then you spend
* one <<consumption_check,consumption>> of magic components. Then you make an
* _intelligence_ check; if successful, you cast the spell. If unsuccessful, the
  spell fizzles, but you can try again later. If nat20, the spellbook loses all
  its <<usage_points>>.
...
====
The <<_tragic_missile_x>> spell is technically not a single spell. It exists
...
---
The <<_ghost_mount_x>> spell is also multiple spells, but since its
...
Of the next `_X_+1` _charisma_ checks you make, you have advantage on checks
that have a strong flirting component or involve physical attraction.

=== Animal Friendship (X)
*Level 1, <<focus>>, <<multilevel>>, <<ritual>>*

Within the next _X_ + 1 hours, all _intelligence_, _wisdom_, and _charisma_
checks made to befriend animals have <<advantage>>. On top of that, there is a
50% chance that an animal that would have otherwise been hostile towards you,
is now simply cautious.

=== Armor (X)
*Level 1, <<multilevel>>, <<ritual>>*

You turn a nonmagical piece of clothing that you wear into a functional armor
piece that provides _X_ <<armor_hit_points,hit points>>. You are skilled at
using the armor.

You can create a cuirass, gauntlets, greaves, and a helmet, but you cannot
create a shield.

The armor becomes nonmagical if it breaks during combat, if you remove it, or
if you sleep.
...
[quote]
Some monsters look like animals. But they're monsters, and are not affected
by this spell.
...
can be interpreted in many ways (drop the thing you’re holding, or drop
<<prone>>, or drop the subject).
...
TIP: Items that have recently been repaired with <<_mend_x>> also glow faintly.
...
dulls all non-magical sources of light within 5+_X_ meters reducing their
...
from an object within _X_ meters that weighs no more than _X_ kg. A nonmagical
<<Armor,armor piece>> can be mended, becoming fully functional, if it provides
no more than _X_ <<armor_hit_points,hit points>>. A nonmagical weapon can be
mended, becoming fully functional, if it takes up less than _X_
...
You target a creature within _wisdom_+_X_ meters and roll an <<adversarial>>
_charisma_ check. If successful, your target has _X_ fewer moves each round for
the next _LVL_+_X_ rounds
