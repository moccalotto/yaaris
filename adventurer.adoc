...
| 1d10 | Item
| 1    | 2d4 gold pieces
| 2    | <<spellbook>> with 1 <<usage_points,usage point>>
...
cast these spells on the fly (this is called <<basic_spellcasting>>), and you
can cast certain spells as rituals (this is called <<basic_ritual_casting>>).
In addition to using spellbooks to cast spells, you can also recharge the books
via <<basic_recharging>>.
...
[reftext="armorer"]
[[armorer]]
*Armorer*: You can repair a piece of broken armor if you have the right tools. See
<<tinkering_tools>> for more information.
...
have advantage on haggle checks (see <<the_haggling_check,The haggling
check>>).
...
[reftext="rider"]
[[rider]]
*Rider*: You acquire a trained horse (for free, as a gift, or similar). You
become proficient at riding horses. You have advantage on all riding-related
checks. You have advantage on <<consumption_check>>for
animal feed. When you reach level 5 your riding proficiency expands to all
rideable land creatures. At level 8, you can ride virtually any tame creature.
...
[reftext="spelunker"]
[[spelunker]]
*Spelunker*: You always have some chalk. You always know the cardinal
directions. You have advantage on checks that involve navigation in caves and
dungeons. One of the ropes you carry does not take up an item slot. Aside from
being unable to see, you do not suffer any debilitating effects from being
alone in total darkness (which can otherwise cause you to suffer from dungeon
madness ???). You have advantage on consumption checks for <<lamp_oil>> and
<<Torch,Torches>>.
...
[discrete]
==== Acquire
Acquiring a treasure means, recovering it (possibly by liberating it from
baddies), and getting it safely back to your base of operations. Since a
hundred coins take up an entire item slot, it can be necessary to make multiple
trips to recover a large treasure.

[discrete]
==== Suitable
A treasure is _suitable_ if it is worth 100 gold pieces times the sum of the
levels of all the characters in the party.
...
[reftext="nat1"]
[[nat1]]
*Rolling a 1*: Checks are always successful if you roll a 1 on your d20, this
*: called a _nat1_.

[reftext="nat20"]
[[nat20]]
*Rolling a 20*: Checks are always unsuccessful if you roll a 20 on your d2*:
...
<<lamp_oil>>) have usage points. Once these items run out of <<usage_points>>,
they’re gone, completely used up.
...
items run out of <<usage_points>>, they stop working, but they can
be repaired.
...
has a maximum number of <<usage_points>>, and a current number of
<<usage_points>>. As a repairable item is used, it loses its
Current <<usage_points>>, and it becomes unusable when it reaches
zero. It can be repaired, but never regain more than its Maximum
<<usage_points>>.

Exactly how a repairable item loses its <<usage_points>>,
and how it is repaired again, depends on the type of item; see the
<<basic_spellcasting>>, and <<weapon_wear>> for more info.
...
Light Melee Weapons::
A light weapon costs 5gp and takes up 1 <<usage_points,usage point>>.

Heavy Melee Weapons::
A two-handed weapon costs 6gp and takes up 2 <<usage_points>>.
...
[quote]
You can attack baddies who are up to 2·_wisdom_ meters away, but if they are
more than _wisdom_ meters away, the <<attack_and_damage,attack check>> is
<<difficult>>.
...
[[rations]]
==== Rations
Can sustain you. You must use one <<consumption>> of rations every
day to stay healthy.

[reftext="iron rations"]
[[iron_rations]]
*Iron Rations*: are a kind of rations that takes up less room so that have
more <<usage_points>> per <<item_slots,item slot>>.

WARNING: if you do not eat, you become <<starving>>.
...
[quote]
yes, if you're wearing a piece of broken armor that you're not using, you
lose a _two_ <<Moves,moves>>; one because you're unskilled, and the other
because the armor piece is broken.

.Armor wear
====
You've just been attacked and rolled a <<nat20,natural twenty>>on your defense
...
[[ritual]]
ritual::
These spells can be cast via <<basic_ritual_casting>>, but can also be
...
<<lvl>> meters of that point glows with a faint light as if on fire. Invisible
creatures and object can now be seen, but attack checks against them remain
<<difficult>>
...
You target a creature in range. If you succeed an <<adversarial>> _wisdom_
check, the creature loses _X_d8 <<hit_points>>, otherwise it loses _X_d4
<<hit_points>>.
...
//
*Nat 1*: If you roll a d20 and it shows the number 1, you rolled a natural (nat) one. +
//
*Nat 20*: If you roll a d20 and it shows the number 20, you've rolled a natural (nat)
twenty. +
//
