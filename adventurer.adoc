...
[%unbreakable]
--
*Strength*: melee attacks, jock stuff. +
*Dexterity*: stealth, melee defense, tricky tasks. +
*Constitution*: carrying stuff, surviving bad trauma. +
*Intelligence*: initiative, magic, tricky tasks. +
*Wisdom*: ranged attack, ranged defense, perception. +
...
[%unbreakable]
--
Roll twice on <<utility_gear_2,the table below>>. If the second roll is the
same as the first roll, reroll it.
...
//-----|-----------------------------
| 1    | 2d4 gold pieces
| 2    | <<spellbook,Spellbook>> with 2 <<usage_points>>
         and a spell of the <<GM,GMs>> choice.
| 3    | <<torch,Torch>>
| 4    | <<spikes,Spikes>>
...
If your throwing weapon is damaged in combat (for instance, by you rolling a
<<nat20>> on an attack check) the weapon loses a <<usage_points,usage point>>.

When a throwable weapon reaches zero <<usage_points>> it is completely used up
...
| Axe     __{melee_ranged}__    |   1  |  1   | 3  gp
| Dart                          |   1  |  4   | 3  gp
| Bow                           |   2  |  -   | 7  gp
...
Torches have 2 <<usage_points>>, and you must make a {consumption}every hour.
...
*Armor: Skilled & Unskilled*:
Adventurer's are typically skilled using light armor pieces. For each piece of
armor you are wearing that you are not skilled at using, you lose one
<<moves,move>>.

*Unarmed Combat*:
Requires two free hands but does not suffer from <<_weapon_wear,weapon wear>>.
Only <<pugilist,pugilists>> are skilled at using these weapons.
...
| {one}     | You deal maximum damage and if you are <<weapon_skills,skilled>>
...
|===
//----------|-------------------------------------
| {twenty}  | You take maximum damage and your best armor piece <<armor_wear,breaks>>.
...
|===
//----------|----------------------------------------------------
| {twenty}  | The spell fizzles and the book loses __all__ its <<usage_points>>.
...
| {one}     | The spell takes effect and no <<usage_points>> are spent.
//----------|----------------------------------------------------
|===
...
__See the <<example_multilevel>> example.__
...
.Cancelling a spell
****
You cannot cancel or stop a spell with a <<duration>> unless the spell has the
<<focus>> or <<trance>> property, or if cancellation is allowed in the spell's
description.
****
...
=== Crackdown (X)
*Level 2+, range: __wisdsom__ meters, <<focus>> (up to <<X>> rounds)*

You call down a bolt of lightning onto a baddie within range. The bolt deals
__[level]__d6 damage. Each round thereafter, at the cost of 3 <<moves>> you can
...
__strength__ check, and it is <<difficult>> if you're not skilled at using your
weapon.

//KEY:
*Attack Check (Ranged)*: A check to hit an opponent. It's an <<adversarial>>
__wisdom__ check, and it is <<difficult>> if you're not skilled at using your
weapon.
...
stats). For instance: when rolling a __strength__ check you roll a d20 and must
roll lower than your __strength__ to succeed.

//KEY:
*Defense Check (Melee)*: A check to avoid being hit by melee attacks. This is
an <<adversarial>> __dexterity__ check.
