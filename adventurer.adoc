...
Both = Difficult::
If one or more conditions make a check <<difficult>>, it remains difficult if
there are other many other circumstances that would have made the check
<<easy>>.
...
To hit a baddie you must roll an attack check. There are two types of attack check:
ranged and melee.

Melee::
When you use melee weapons or unarmed combat, the attack check is an
adversarial _strength_ check.

Ranged::
When you use ranged weapons, the attack check is an adversarial _wisdom_ check.

[IMPORTANT]
====
* If you attack someone who is 2 or more levels higher than you, your attack check is difficult.
* If you are not <<Weapon Skills,skilled>> at using your weapon, your check is difficult.
====
...
Dead, Dying, and Comatose:: These conditions are described in detail the
<<CONDITIONS>>
...
[[dungeon_mad]]
Dungeon Mad::
You've been in complete darkness in a dungeon too long. Every hour you must
make a _wisdom_ check. If you fail, you lose 1 point from a random stat.

[reftext="encumbered"]
[[encumbered]]
Encumbered:: 
All checks become <<difficult>>, meaning that you
...
[[poisoned]]
Poisoned::
You temporarily lose 1d4 points of a random stat every minute. After rolling
your stat loss, you make a _constitution_ check with
<<disadvantage,disadvantage>>. If successful, the poisoned condition is
removed. You die if any of your stats are reduced to zero. If you die from
poison, only Greater Revival can restore you back to life.

[reftext="Restrained"]
[[restrained]]
Restrained::
Your hands and feet are restrained. You have only one <<moves,move>> each round.
Depending on the circumstances, the GM might allow you to try and escape your
bonds.
...
*Level 1, <<multilevel>>, <<ritual,ritual>>, duration: _X_ days*
...
=== Animal Friendship (X)
*Level 1, <<focus>>, <<multilevel>>, <<ritual,ritual>>*
...
=== Armor (X)
*Level 1, <<multilevel>>, <<ritual,ritual>>*
...
You target a point within _wisdom_ meters.
Every object and creature within 5 meters of that
point glows with a faint light as if on fire.
Invisible creatures and object can now be seen,
but attack checks against them are <<difficult>>
...
=== Heal (X)
*Level 1+, <<ritual,ritual>>, <<multilevel>>, range: 3+_X_ meters*

You heal a creature within range. It recovers _X_ recovery dice.

=== Hurt (X)
*Level 1+, <<multilevel>>, <<ritual,ritual>>, range: _X_+3 meters*
...
=== Purify Rations (X)
*Level 2, <<ritual,ritual>>, <<multilevel>>*
