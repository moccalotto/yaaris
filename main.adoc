...
level 1 and max out at level 10. Incidentally, spells, baddies, and certain
items also have levels
...
Total item slots:: is the maximum number of items you can carry. This number is
the highest of your _strength_ and _constitution_.
...
Moves:: is the number of meters you can move in a round (and other useful
things).
// }}}
...
==== Spellcasting
You can use <<Spellbooks,spellbooks>> to cast <<basic,basic>> spells. You can cast these
spells on the fly (this is called <<Basic Spellcasting>>), and you can cast
certain spells as rituals (this is called <<Basic Ritual Casting>>). In addition to
using spellbooks to cast spells, you can also recharge them via
<<Basic Recharging>>.
...
Hero. Choose a hero class from the xref:heroes#[]
...
hero, and you can find all the info about heroes in the xref:heroes#[Hero’s
Guide to doom & destruction & stuff],
...
=== Exchange Rates
The price for one gold piece is 100 silver pieces, and the price for a silver
piece is 100 copper pieces. Normally, only rich persons deal in gold, merchants,
small businesses, and small farms deal in silver, and unskilled labor deal mostly
in copper.
...
=== Basic Spellcasting
Adventurers can cast <<basic,basic>> spells (all spells in this document are <<basic,basic>>).
...
Some spells can be without exhausting the spellbook, but it takes more time to
do so.
...
//{{{SPELLS
== SPELLS
All spells listed in this section are <<basic,basic>> and can be used by adventurers.
...
An ally within _X_ meters gets advantage on their next Attack- or Defense check,
provided it occurs within _X_+1 rounds.
...
with a level higher than _X_ can detect the item’s magic, and that this spell has
been used to hide it.
...
Observers with a level higher than _X_ can detect that this spell has been cast,
and that the item is nonmagical.
...
anywhere you can see within 10·_X_ meters, and it must remain within that radius
at all times. You can move the phantasm 1·_X_ meters per round. The spell ends
after 5·_X_ minutes, if you stop focusing on it, or if something substantial
touches the phantasm (heavy smoke, sandstorm, a creature, etc.).
...
You conjure a large vaguely bowl-shape plate that can carry as many item slots
as your _strength_ score. It levitates about a meter above the ground, and can
move half as fast as an adventurer. It is able to “climb” stairs and hills, but
cannot scale walls or steep cliffs.
...
struck, the baddie takes _X_ d4 damage, and then you make an adversarial
_charisma_ check; if successful the baddie becomes <<demoralized,demoralized>>.
...
Makes a piece of chalk magical such that it makes marks only you can see.
Except for being invisible, the marks behave as normal chalk. The chalk stops
being magical once you stop focusing on the spell, or after _charisma_ hours.
...
For example: when rolling a _strength_ check you roll a d20 and must roll lower than your _strength_ to succeed.
